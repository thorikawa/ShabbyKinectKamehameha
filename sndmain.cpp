...
int Callback(const void *input,
             ...
             void *userData)
{
  OurData2 *data = (OurData2 *)userData; /* we passed a data structure
...
int main2()
{
  OurData2 *data = (OurData2 *)malloc(sizeof(OurData2));
  PaStream *stream;
  PaError error;
  ...
  OurData2 *data2 = (OurData2 *)malloc(sizeof(OurData2));
  PaStream *stream2;
  data2->position = 0;
  data2->sfInfo.format = 0;
  data2->sndFile = sf_open("kamehameha_1.wav", SFM_READ, &data2->sfInfo);
...
same number of channels as our sound file */
  outputParameters.sampleFormat = paInt16; /* 32bit int format */
...
callback knows what's up */
  ...
  if (error)
  {
    printf("error opening output2, error code = %i\n", error);
    Pa_Terminate();
    return 1;
  }
